    ...
    function httpReqWeatherAsync(url, callback) {
        ...
        httpReqWeather.onreadystatechange = function() {
            if(httpReqWeather.readyState == 4 && httpReqWeather.status == 200) {
                ...
                var tempFahren = Math.round(1.8 * (temperature - 273) + 32);
                var wind = jsonWeather.current.wind_speed;
                var desc = jsonWeather.current.weather[0].description;
                ...
                descElement.innerHTML = `<p>Today, the weather is ${desc}. With temps to ${tempFahren}&deg; and winds up to ${wind} mph.</p>`;
            }
        }
        httpReqWeather.send();
    }
